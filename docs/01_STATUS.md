...
> 2026-09-24 (row 18: 2024 pilot passed, V43; output size measured, full production estimated at 12.1-17.4 TB, V44). 2026-09-23 (row 18: 2024 pilot submitted after a failed first attempt; `crab/submit_crab.py` fixed, `05_troubleshooting.md` A22; batch 6 recorded in `09` 21, ledger V38–V42). Before that 2026-09-22 (batches 3-4 folded in; user decisions 22t: 2018UL v15 + 2024 first, `TTWJetsToLNu` dropped; submission batch = workspace RUNBOOK 10). Keep this current; details/why live in `03_DECISIONS.md` and `02_CHANGELOG.md`.
...
- **2018UL campaign — DAS scan DONE, config generated (2026-07-26):**
  ...
  `script/das/das_ul18_scan_20260726_1657.log`) — all **61 MC primary datasets**
...
- **2018UL FULL production — SUBMITTED 2026-07-27.**
  ...
  **85 tasks / 7,466 jobs / 6.74 TB expected**, `--preflight` 35 PASS / 0 FAIL
  ...
  - This production did **not** wait for the ttHH categorization work: the
    ...
    two campaigns are independent (workspace `archive_2026/RUNBOOK_UL18_to_controlplots.md`
    §0).
...
- **Output filename = `forgedNtuple.root` (D-F executed 2026-07-26).**
  ...
  **both** `forgedNtuple*` and `slimmedNtuple*`, because pre-2026-07-26
  ...
  `archive_2026/00_CONTEXT_ExpandedTtbarId_NtupleForge_Migration.md` §2.3.
- **DEFERRED (2026-07-27, user decision) — `modules/expandedTtbarIdInjector.py`:**
  ...
  `archive_2026/00_CONTEXT_ExpandedTtbarId_NtupleForge_Migration.md` §4) but **no code has
...
**다음 행동 (2026-09-22 기준; 09-16 저녁 표를 갱신).** 근거와 이력은 22a~22s 에 있다. 여기에는 *무엇을 할 차례인가*만 적는다.
...
| 11 | **트리거 결정 (2016, Run 3)**: 사용자 09-17 "Run 3 는 아직 정한 바 없음". 생산에는 영향 없음(wildcard 가 HLT 계열 전부 보존, 용량 문제 아님). analyzer 확장 때 `HLT_REQUIRED` 채움. **2016 후보 = AN2019_094 Table 24**(`HLT_PFHT450_SixJet40_BTagCSV_p056` OR `HLT_PFHT400_SixJet30_DoubleBTagCSV_p056` OR `HLT_PFJet450`, 효율 기준 `HLT_IsoMu24`; 9 era 파일 + MC 두 half 전부에 있음, `08` §7.4) | 보류 | 22m, 22n |
| 12 | **analyzer 측 4 건**: `Jet_jetId`/`Jet_puId` 재계산(08 §3.4, 변화 없음), `Flag_METFilters` 부재(Summer24 MC·2025 Data), `L1PreFiringWeight` Run 3 미적용, **2018A 전체가 옛 six-jet 쌍만 가짐** vs `requireTriggerBranches2018_()`(22n 의 선택지 3 개, 사용자 결정) | tempTTHH | 22m, 22n |
...
| 18 | **제출 배치 (RUNBOOK §10)**: 2024 재스캔(`TTWJetsToLNu` 제외) + 2018UL v15 스캔 → 초안 4 개 → `crabConfig/config_ttHH{2018UL,2024}_v15_had_{MC,Data}.yaml` → 컨테이너에서 era·tier 별 500 event 로컬 점검 4 → preflight 4 → 파일럿(2024 MC `ZZ` + Data `JetMET0_Run2024H`) → 파일럿 확인 뒤 전체 4 config 제출 → `--report` 요약을 저장소에. **09-23**: 로컬 점검 4, preflight 4 + 파일럿 preflight 2 는 기대대로(FAIL 0; 커밋 `3cb789b`·`72bd16f`, 기록 `09` 21 절, 원장 V38–V41). 파일럿 1 차 제출은 myproxy 재위임의 pass phrase 프롬프트가 붙여 넣은 다음 줄을 가져가 실패했고 wrapper 는 exit 0 이었다 (`05_troubleshooting.md` A22; wrapper 수정은 맥 커밋 `c7cbe95`, V42). 재시도로 **파일럿 2 task 제출 완료**(`260923_081917:junghyun_crab_ZZ`, `260923_082524:junghyun_crab_JetMET0_Run2024H_MINIv6NANOv15_v2`). **09-24 파일럿 통과**(158/158 done, event 합계 = DAS, 스키마가 09-23 로컬 점검과 일치; V43). 출력 크기 MC `ZZ` 1.056 / Data `JetMET0` 0.780 kB/event → 전체 추정 12.1~17.4 TB(V44). lxplus 는 `288b947` 까지 pull. 남은 것: T3_KR_KNU `/store/user/junghyun` 여유 확인(RUNBOOK §10 [7b]) → 전체 제출 [8] | 사용자 (lxplus) | 22t, A22 |
| 18a | ~~**Run 3 첫 config 초안 검토 (2024 had)**~~ 09-22 검토 끝: `script/drafts/config_das_ttHH_2024_v15_20260918_0803_{MC,Data}.yaml.draft`(MC 61 = 19,322 files 13.95 TB, Data 32 = 7,811 files; jobID `ttHH2024_v15_had_{MC,Data}_v1`, 브랜치 목록 `branch_hadronic_2024_v15_{MC,Data}.txt`). 검토 항목: Muon0/1 포함 여부(registry `had,lep`), `units_per_job`, 출력 사이트, `TTWJetsToLNu` mg35x 수용(표 4). 통과하면 `crabConfig/` 로 복사 후 preflight. **2025 초안도 나왔다(09-22)**: `script/drafts/config_das_ttHH_2025_v15_20260922_0755_{MC,Data}.yaml.draft`. 2025 MC 초안의 dataset 61 개는 2024 MC 초안과 **완전히 같다**(2025 MC 캠페인이 없어 Summer24 를 같이 씀) → MC 는 한 번만 생산하고 2025 는 **Data 초안만** 제출 대상(Data 32 행 = JetMET0/1 + Muon0/1 × PromptReco B~G, C·F 는 `-v1`+`-v2`, 6,607,369,332 ev, 18,494 files). 검토 항목은 2024 와 같다 | 사용자 | 22r, 22s, ttHH/03 §6 1 |
...
22a. ~~**event/file 수 스캔**~~ **DONE 2026-09-16**: `das_scan.sh --era 2024|2025 --nano v15 --workstream had`, lxplus `runlog.sh` 기록(`script/runlogs/run_das_scan_202?_had_*.log`), 로그 `script/das/das_ttHH_2024_v15_20260916_0859.log`·`das_ttHH_2025_v15_20260916_0900.log` 커밋(`a781cb3`). 65 키(MC 61 + DATA 4), EXACT 64, NOT_FOUND 1 = `TTWJetsToLNu`; DATA PD 4 × 8 dataset, DBS `-vN` 꼬리가 PD 마다 다름. 상세 `09_v15_migration_log.md` 17 절, `ttHH/03_run3_plan.md` §6 1.
...
22k. **Run 2 범위 = full Run 2 (09-11 밤, 사용자 결정)** — 2016 preVFP/postVFP 를 `das_inventory.sh` 로 점검(v15 2 + v9 2, 로그 커밋). 네 Run 2 v15 캠페인의 `NOT_FOUND` 집합이 **동일**(diff 0) — 같은 5 종이 빠져 있고 나머지는 다 있다. 요청 = 5 종 × 4 era-half = **28 datasets ≈162M**(2016 은 09-16 MiniAODv2 부모 기준 27.2M/27.1M; 09-11 의 v9 수치 27.1M/27.1M 을 대체). registry: ttHH 62 행 ERAS 4 era-half 로 확장(era 당 had 45), QCD-HT PRIMARY 를 PSWeights 이름으로 교체(네 캠페인 EXACT) → 2017/18 v15 의 QCD-HT NOT_FOUND 도 해소(27 → 20). 남은 것: ~~2016 MiniAODv2 부모 수~~(09-16 DONE: 두 캠페인 136 키 전부 EXACT, `script/das/inventory_dumps/das_inventory_ul16{pre,post}_miniaodv2_20260916_*.tsv`, 원장 V12), `TTTW` 전하 분할 KEY 2 개 + xsec, 데이터 PD 2016 행(v15 JetHT era 문자열 9 개는 `run_discover_ul16_jetht_v15_*.log` 로 확정, `inventory_manifest_run3_2016.txt` 에 반영). `03_DECISIONS.md` D-2026-09-11-run2-scope-2016, `09_v15_migration_log.md` 16·17 절.
...
22n. **2018A 초기 HLT 메뉴 (09-16 발견, 한 파일 측정)**: `/JetHT/Run2018A-UL2018_NanoAODv15-v2` 첫 파일(`inv_2018A_v15_Data.tsv` 의 `# source=`)에 analyzer 2018 필수 경로 `HLT_PFHT400_SixPFJet32_DoublePFBTagDeepCSV_2p94`·`HLT_PFHT450_SixPFJet36_PFBTagDeepCSV_1p59` 가 **없고** 2017 임계값의 DeepCSV 판(`…380_SixPFJet32_DoublePFBTagDeepCSV_2p2`, `…430_SixPFJet40_PFBTagDeepCSV_1p5`)만 있다. B 는 둘 다, C·D 는 새 것만. `requireTriggerBranches2018_()` 이 FATAL 하는 조건이라 확인 필요: 그 파일의 run 범위(`dasgoclient -query "run file=<LFN>"`), v9 2018A 생산에서 어떻게 지나갔는지, analyzer 가 2018A 초기 run 에서 어느 경로를 써야 하는지(AN 의 2018 트리거 정의). `check_branchlist.py` `HLT_ERA_CONDITIONAL["2018"]` 에 두 경로를 넣어 정보로 출력한다. **09-17 측정**: 그 파일의 run 범위는 316058–316719, dataset 은 315257–316995(`run_runs_2018A_firstfile_20260917_060634.log`). **09-18 확정 (`probe_hlt_path_by_run.py`, `08` §7.4, 원장 V27–V29)**: 2018A 의 run 별 표본 파일 9 개(합쳐 315257–316995 = era 전체)가 **전부** 옛 쌍만 갖고, 2018B 표본 6 개(317080–319310)는 전부 네 경로를 가진다; v9 인벤토리도 같다. 스키마만으로는 새 쌍이 316995 와 첫 2018B 파일(317080–317696) 사이에 들어왔다는 것까지 알고, 2018A 파일은 전부 analyzer 의 2018 필수 쌍이 없다(한 파일·초기 run 문제가 아니라 era 문제). **09-19 AN2019_094 대조(`Materials/TTHH/TTH_AN/AN2019_094_v20_ttHAnalysis.pdf` §3.1.4, Tables 28–30; `08` §7.4 표 2 개, 원장 V34)**: 진입 run 은 **317509(2018B 안)**. ttH(bb) 분석은 2018 데이터를 세 기간의 OR 로 썼다(A 315252–315974: `..._SixPFJet40_PFBTagCSV_1p5` + `..._2p2`; B 315974–317509: `..._SixPFJet40_PFBTagDeepCSV_1p5` + `..._2p2`; C 317509–end: `..._1p59` + `..._2p94`; 세 기간 모두 + `..._4p5` + `HLT_PFHT1050`), MC 는 기간 C 구성 하나, SF 는 run 평균. 우리 analyzer 의 `HLT_REQUIRED["2018"]` 는 정확히 기간 C = MC 구성이다. AN 의 경로는 전부 우리 2018 인벤토리에 기간 구조대로 있다(기간 A 의 CSV 판은 run < 315974 를 덮는 2018A 파일에만). **사용자 결정 항목(analyzer, tempTTHH)**: (a) AN2019_094 방식 = run 으로 기간 A/B/C 를 나눠 그 기간의 경로 OR (2018A 는 기간 A·B), (b) 두 쌍의 OR 을 2018 전체에 적용(C·D 에는 옛 쌍 branch 가 없으므로 부재 branch 를 false 로 읽는 처리가 필요), (c) 후처리 모듈이 없는 HLT branch 를 false 로 채워 analyzer 의 `requireTriggerBranches2018_()` 을 안 고침((a)·(b) 어느 쪽과도 결합 가능). **AI 권고: (b) + (c)** (09-19 정리: 메뉴에 없던 경로는 fire 할 수 없고 NanoAOD 는 비활성 경로 branch 를 false 로 채우므로, 7 경로 합집합의 OR 은 사건마다 그 기간 경로의 OR 과 같다; AN 도 SF 를 run 평균으로 냈다. 기간 표는 SF 측정·문서용 장부이고 선택 논리는 아니다. D-2026-09-18-2018A-trigger). **(c) 는 '선언된 부재' 형태여야 한다**: eventBuffer 는 없는 branch 를 0 으로 두고 넘어가므로(무음 실패, `requireTriggerBranches2018_()` 가 막는 바로 그것) 일괄 false 채움은 안 되고, 허용 목록 + 파일별 로그로. 2018 기간 B 의 6J1T `HLT_PFHT430_SixPFJet40_PFBTagDeepCSV_1p5` 는 eventBuffer.h 에 선언이 없어 추가가 먼저다. 2017 6J1T prescale(run 299337–300999)은 analyzer·ntuplizer 어디에도 명시 처리가 없고 run 평균 trigger SF 가 흡수한다(AN 과 같은 방식; 세부는 결정 항목). 선행 확인: AN-2022/122 가 2018A 를 어떻게 다뤘는지, 그리고 2018A 데이터로 analyzer 를 돌린 적이 있는지(있었다면 어떻게 통과했는지). v9 2018 config(`config_ttHH2018UL.yaml`)는 `branch_keep_all.txt` 라 ntuple 생산 단계에서는 걸릴 것이 없고, 문제는 analyzer 단계에만 있다.
...
22u. **정리 (09-22, 사용자 요청 "쓸데없는 로그·문서 정리")**: `script/` 의 산출물 64 개를 `script/das/`(스캔·probe·discover 로그), `script/das/inventory_dumps/`(전수 덤프 30), `script/drafts/`(review·config 초안 8) 로, 루트 느슨한 파일 5 개는 삭제 1(`smoke.log`)·이동 4; smoke 산출물 3 개 삭제. 워크스페이스 루트는 문서 4 개만 남기고 6 개를 `archive_2026/`, 발표 자료 3 개를 `Materials/slides/` 로. 문서·도구의 경로 전부 치환(CHANGELOG 09-22 (3), `script/README.md`). 번호 문서 12 개(`docs/`)는 기록 체계라 그대로.
