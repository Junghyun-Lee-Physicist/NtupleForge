...
voms-proxy-init --voms cms (If accessing remote files)
...
ROOT files or .txt lists.

input.root

--output-file

Merge all outputs into this filename.

merged.root

-c, --cut

TTree cut string applied first.

"nJet>0"

-b, --branch-selection

File with keep/drop rules.

branches/keep_and_drop.txt

-I, --imports

Modules to load. Format: pkg.mod (implies modules) or pkg.mod:NAME.

modules.jets_met:MODULES
...
Do not write ROOT file (testing).
